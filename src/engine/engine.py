...
class Engine:
    ...
    def clip_forward(self, batch: Tuple[Tensor, Tensor]) -> Optional[Tensor]:
        ...
        labels = torch.diag(torch.ones(logits_image.size(0))).to(self.device)
        loss_img: Tensor = self.criterion_clip(logits_image, labels)
        loss_text: Tensor = self.criterion_clip(logits_text, labels)
        loss = loss_img + loss_text
        tp, fp, fn = compute_f1_batch(logits_image, labels)
