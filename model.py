...
class CLIP(nn.Module):
    ...
    def __init__(
        ...
        overall_dim - overall dimension
        """
        super().__init__()
        self.__inference_text_embedding = None
        self.model_image_embedding, output_dim_img = configuration_image_model(name_image_embedding)
        self.model_text_embedding, output_dim_text = configuration_text_model(name_text_embedding)
    ...
    def forward_img_part(self, img: torch.Tensor) -> torch.Tensor:
        image_embedding = self.model_image_embedding(img)
        normalized_img_emb = self.linear_image(image_embedding)
    ...
    def forward_txt_net(self, txt: torch.Tensor) -> torch.Tensor:
        text_embedding = self.model_text_embedding(txt)
        normalized_text_emb = self.linear_text(text_embedding)
    ...
    def forward(
        ...
        if image_features is None:
            image_features = self.forward_img_part(img)
        ...
        (
            logits_image,
            logits_text
        ) = CLIP.forward_cosine_similarity(
            image_features,
            text_features
        )
    ...
    def inference(
        ...
        if self.__inference_text_embedding is not None and not is_rewrite_classes:
            inference_text_embedding = self.__inference_text_embedding
            logits_image, _ = self.forward(image, text, text_features=inference_text_embedding)
        else:
            logits_image, *_, text_features = self.forward(image, text, is_raw_output=True)
            self.__inference_text_embedding = text_features
...
def configuration_image_model(name_model: str) -> Tuple[nn.Module, int]:
    if name_model == 'mobilenet_v3':
        ...
        raise ValueError('Please type right model name')
